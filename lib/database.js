...
module.exports = {
	...
	applyRule: function(ruleId) {
		...
		return db.rule.findById(ruleId)
			.then(rule => ({
				bucketId: rule.bucketId,
				where: db.buildFilter({ bucketId: null, account: rule.account, search: rule.search })
			}))
			.then(({ bucketId, where }) => db.transaction.update({ bucketId }, { where }))
