...
class Format extends stream.Transform {
	constructor(options) {
		...
		this._data = (options.regex && options.data && this.parsable() !== false) ? options.data(options.regex.exec(this._filename)) : {};
	...
	_transform(row, encoding, callback) {
		...
		if(this._columns) {
			this.parsable((row.length === this._columns.length) && (!this._header || index !== 0 || row.join(',') === this._columns.join(',')));
	...
	parse() {
		...
		return new Promise((resolve, reject) => {
			...
			me.on('error', () => resolve(me.parsable()));
		});
	}
}
